...
/**
 * Gets web pages for many technologies but not javascript (use ghost.py or another program for this) 
 * 
 * Technology Currently Supported:
 * 
 * html/xhtml
 * ASP.net
 * java server faces (.jsf and .html)
 * java server pages (.jsp and .html)
 * limited flash
 * base 64/gzipped data
 * 
 * Capabilities:
 * 
 * unzip gzipped data/base64 data
 * rezip gzipped data/base64 data
 * encode urls
 ...
 * get images via http/https
 *
 * Returns:
 * String of html
 * 
 * Uses:
 * java.nio for standard pulls
 * java.net for everything else
 * 
 * For use when content-lengths are known or can be deciphered.
 * Can also be used to get the viewstate, html, and event validation code.
 * Contains a base 64 decoder for decoding viewstates into useable information. (custom decoder removes all unknown characters)
 * 
 * By: Andrew S. Evans
 * 
 */
public class html_grab {
	...
	public html_grab(String[] inheaders, String[] invalues,String inmethod,String inurl){
		//TODO actual constructor
	...
	public html_grab(String[] inheaders, String[] invalues,String inmethod,String inurl,String inurl_params){
	...
	public String get_server_faces()
	{
		...
		if(html==null)
		{
			...
			return (get_state(html,"(?<=id=\"javax.faces.ViewState\" value=\").*?\\/>")).replaceAll("\"|>", "");
		}
	}
	...
	public String get_server_faces_decoded()
	{
		...
		if(html==null){
			...
			try{
			ret_string=new String(decode(get_state(html,"(?<=id=\"javax.faces.ViewState\" value=\").*?\\/>")),"UTF-8");
			}catch(UnsupportedEncodingException e)
			{
				e.printStackTrace();
			}
	...
	public String get_decoded_viewstate()
	{
		...
		try{
			ret_string=new String(decode(get_state(html,"(?<=id=\"__VIEWSTATE\" value=\").*?\\/>")),"UTF-8");
		}catch(UnsupportedEncodingException e)
		{
			e.printStackTrace();
		}
	...
	public String get_viewstate()
	{
		...
		return get_state(html,"(?mi)(?<=viewstate.).*?(?=\\|)|(?<=id=\"__VIEWSTATE\" value=\").*?\\/>|(?<=name=\"__VIEWSTATE\" value=\").*?\\/>");
	}
	...
	public String get_event_validation()
	{
		...
		return get_state(html,"(?mi)(?<=__EVENTVALIDATION.).*?(?=\\|)|(?<=id=\"__EVENTVALIDATION\" value=\").*?\\/>|(?<=name=\"__EVENTVALIDATION\" value=\").*?\\/>");
	}
	...
	private static byte[] decode(String s){
        ...
        //set up the byte array which is going to be 75% of base 64 - the difference
        ...
        //letters are represented by two bytes corresponding to ascii but one is generated per loop
        int index = 0;
        for(int i=0; i< s.length(); i+=4){
            ...
            //place the proper byte in the array with shifting first and second bits
            ...
            //repeat the mask operations bit shift second <-- 4 OR 3rd shift -->2 AND mask
	...
	{
		//TODO encapsulate the get state for security, returns the state trimming whitespaces
		String state=null;
	...
	/**
	 * Gets a page using custom verification for self-signed certificates.
	 * Use Sparingly and wisely. Be Careful. This is a disclaimer.
	 *@return cookies
	 */
	private String get_manager()
	{
		//TODO Use a Trust Manager that accepts only a Certain Host Provider for Self-signed certificates
		//All certificates must match the signing authority and host. There is no leeway here!
		//Worst case scenarios should use this generic method. Otherwise try adding the cert to a trust store
			...
			try{
				...
				/*
				 * This pretty much overrides the trust manager and allows for the use of personal verification
				 * This code verifies the host, authority, and certificates manually and with the aid of a decryption tool.
				 * If the site is not trustworthy, do not use this method.
				 */
				
				//creates an all-trusting trustmanager which allows for self-verification
	        ...
	        //initializes
			ssl_context_handler.init(null, tm, new java.security.SecureRandom());
	        HttpsURLConnection.setDefaultSSLSocketFactory(ssl_context_handler.getSocketFactory());
			...
			if(method != null)
			{
				...
				//if the method is a POST, set the appropriate headers and write out the POST parameters to the
				//output stream
				if(method.compareTo("POST")==0)
				{
					conn.setDoOutput(true);
			...
			/*
			 * This is where the socket is vulnerable and bits should not be accepted for a MIME attack;etc.
			 * Verification can occur here sinc the certificate is obtained here. Do not proceed without a valid certificate
			 * from a trusted authority.
			 *
			 *For non-Pentaho programs, it may be ok to place this code in a separate method although that 
			 *would require passing the connection information between class methods in a public class.
			 */
			//loop through the certificates verifying that each is valid and contains a trusted host and authority
			...
			for(java.security.cert.Certificate cert: certs)
			{
				...
				if(cert.getType().compareTo("X.509")==0 | cert.getType().compareTo("X509")==0)
				{
					
					
					//certify from preset authorities after getting X509 typed cert
					...
					//get the IssuerX500 Principal which should match the other principal
					//requires the existence of an OU
					...
					//verify that the issuer, issuer principal, and peer principal are valid
					//this can prevent MIME attacks for the most part but BE CAREFUL
					
					//VERIFY HERE
					//System.out.println(CN+" "+Issuer);
					try{

						if(CN.compareTo(host)==0 &(Issuer.compareTo(authority) == 0 & IssuerX500.compareTo(authority) == 0))
						{
						...
						{
							//if the authority or host names do not match then the Invalid Exception is thrown
					...
					//If this works, please try using the getSSL() method which uses less code
					try {
			...
			//personal verification for the certificate to handle self signed X509 certificates
			//a host name must be provided
			//a certificate signer name must be provided
			//to be secure, these must be found elsewhere (to download the certificate go to the web page in a browser and click the lock icon or etc.)
	...
	/**
	 * Return a random user agent including
	 * Mozilla 4+, Opera 9.80, and the Enigma Browser
	 * @return useragent
	 */
