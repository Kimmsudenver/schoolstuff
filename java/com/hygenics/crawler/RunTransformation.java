...
/**
 * This Class Runs an Existing Pentaho Transformation that is Saved in .ktr or .xml format
 * 
 * @author aevans
 */
	...
	public void run()
	{
		...
		for(String fpath:fpaths)
		{
			try{
				...
				log.info("Starting ktr PATH: "+fpath+" @ "+Calendar.getInstance().getTime().toString());
				...
				if(trans.getErrors()>0)
				{
					...
					log.info("Transformation Returned: "+trans.getErrors()+" @"+Calendar.getInstance().getTime().toString());
				}
				
				log.info("Kettle Complete: Cleaning Up");
				transMeta.clear();
				trans.cleanup();	
				log.info("Transformation Complete @ "+Calendar.getInstance().getTime().toString());
