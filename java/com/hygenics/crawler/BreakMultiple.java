...
/**
 * Breaks apart Records with Multiple Entries.
 * This may be the result of having run a looped 
 * or multiregex query. Also performs Replacements.
 * This class works for one table. Utilizes split queries. 
 * 
 * I found that using the DAO template and bonepc pool has the same effect
 * as pooling jdbc connections and running more connections.About ~22.4 seconds (22.35 average) 
 * for ~12000 posts and ~1000 pulls along with parsing using ~1700 regex operations, all with fork/join pools.
 * Don't go too high. I swamped the server with connections. It hates me for a while.
 * 
 *You can specify a primary key and foreign key in this step if desired. The primary key is usesful in pre-processing data
...
public class BreakMultiple {
	...
	//the array list containing the tables to take from and a mapping of positions and target variables
		...
		public void run() {
			...
			if(this.json != null){
				...
				for(String str: this.json)
				{
					...
					else if(table.compareTo(jmap.get("table").asString().trim())!=0 | jmap.size()>numvals |jmap.size()<numvals |missingkey==true)
					{
	...
	private class Break implements Callable<ArrayList<String>>
	{
		...
		public Break(final boolean repeatkeys, final getDAOTemplate template, final String row, final String token, final String replacementPattern, final Map<String,String> positions,final String date, final String table, String offenderhash,final int maxpos, final boolean genhash)
		{
			this.maxpos=maxpos;
			this.token=token;
			this.template=template;
			...
			if(positions != null)
			{
				...
				{	
					...
					if((i%this.maxpos)==0 & i != 0)
					{
						...
						if(json != null)
						{
							json+=",\"table\":\""+table+"\",\"date\":\""+date+"\",\"offenderhash\":\""+offenderhash+"\"}";
							retrows.add(json);
						}
					...
					if((j>=positions.size() & repeatkeys ==true)| j<positions.size())
					{
						...
						if(positions.containsKey(Integer.toString(j).trim()))
						{
							...
							if(positions.get(Integer.toString(j)).trim().toLowerCase().contains("skip")==false){
								json=(json==null)?"{\""+positions.get(Integer.toString(j).trim()).replaceAll((replacementPattern+"|\""),"")+"\":\""+vars[i].replaceAll((replacementPattern+"|\""),"")+"\"":json+",\""+positions.get(Integer.toString(j).trim()).replaceAll((replacementPattern+"|\""),"")+"\":\""+vars[i].replaceAll((replacementPattern+"|\""),"")+"\"";
							}
							j++;
						}
	...
	/**
	 * Get the user specified field that cannot be null. This is used in posting data to the database, specifically in the
	 * parallel post class.
	 * 
	 * @return
	 */
	...
	/**
	 * Set the user specified field stating what cannot be null. This is used in posting data to the database, specifically in the
	 * parallel post class.
	 * 
	 * @param notnull
	 */
	...
	public void checkTable()
	{
		...
		if(this.template.checkTable(targettable, tablearr[0])==false)
		{
			...
			this.template.createTablebyMap(targettable, tablearr[0], tpositions,key,foreignkey,fkeyref);
		}
		else if(truncate){
	...
	public void run()
	{
		...
		{
			condition=" WHERE "+idcolumn+" > "+Integer.toString(offset+((int)Math.round(pullsize/qnum)*i))+" AND "+idcolumn+" <= "+Integer.toString(offset+((int)Math.round(pullsize/qnum)*(i+1)));
		...
		while(rows.size()>0)
		{
						...
						if(r.contains(mustcontain))
						{
							if(cannotcontain != null)
							{
								if(r.contains(cannotcontain)==false)
								{
									...
									//final getDAOTemplate template, final String row, final String token, final String replacementPattern, final Map<String,String> positions,final String date, final String table, final String offenderhash
									if(rowmap.size()>0){
										collect.add(new Break(repeatkeys,template,rowmap.get(rowcolumn).asString(),token,replacementPattern,positions,(Calendar.getInstance().getTime().toString()),targettable,rowmap.get("offenderhash").asString(),maxpos,genhash));
									}
								}
							}
							else
					...
					else
					{
						...
						if(cannotcontain != null)
						{
							if(r.contains(cannotcontain)==false){
								...
								//to ascend you must die, to die you must be crucified; so get off your -- cross so that we can nail down the nex martyr
								//final getDAOTemplate template, final String row, final String token, final String replacementPattern, final Map<String,String> positions,final String date, final String table, final String offenderhash
								if(rowmap.size()>0){
									collect.add(new Break(repeatkeys,template,rowmap.get(rowcolumn).asString(),token,replacementPattern,positions,(Calendar.getInstance().getTime().toString()),targettable,rowmap.get("offenderhash").asString(),maxpos,genhash));
								}
			...
			{
				//change condition
				condition=" WHERE "+idcolumn+" > "+Integer.toString(offset+((int)Math.round(pullsize/qnum)*conn))+" AND "+idcolumn+" <= "+Integer.toString(offset+((int)Math.round(pullsize/qnum)*(conn+1)));
