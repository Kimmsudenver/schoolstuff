...
/**
 * Creates tables. Useful for creating Pentaho Tables in a drop, recreate schema situation.
 * @author asevans
 *
 */
		...
		for(String table: tables.keySet()){
			...
			if(tables.get(table).size()>0)
			{
				...
				for(String attr: tables.get(table)){
					attrs=(attrs==null)?attr+" text, id serial PRIMARY KEY NOT NULL":attrs+","+attr+" text";
				}
