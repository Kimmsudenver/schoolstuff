	...
	public void run()
	{
		...
		for(String fpath:fpaths)
		{
			try{
				...
				log.info("Starting Pentaho Job kjb PATH: "+fpath+" @ "+Calendar.getInstance().getTime().toString());
				...
				if(j.getErrors()>0)
				{
					...
					log.info("Pentaho Job Returned: "+j.getErrors()+" @"+Calendar.getInstance().getTime().toString());
				}
