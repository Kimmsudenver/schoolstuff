...
/**
 * Dumps a File From command Line using specified delimeter, database,
 * username, and password in pgsql. Takes in a List<String> of tables to dump.
 * If not using pgsql, contact me and use dump to text for small files. It has reasonable speed and will not crash for files
 * under 80,000 rows. 
 * 
 * @author aevans
 *
...
public class CommandDump {
	...
	protected class ToFile extends RecursiveAction
	{
		...
		public void compute()
		{
			...
			conn.CopyOut(sql.trim(),(select.trim()+Calendar.getInstance().getTime().toString().trim().replaceAll(":|\\s", "")+".txt").trim());
		}
	}
	...
	public void run()
	{
		...
		for(String k:keys)
		{
			...
			String sql="COPY  ("+selects.get(k)+") TO STDOUT DELIMITER '"+delimiter.trim()+"'  CSV HEADER";
