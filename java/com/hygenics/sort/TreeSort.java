...
/**
 * Another sorta big data sort. This is a tree sort. It creates a 
 * binary tree from the data. You can get the treenode data or a sorted 
 * array. Worst case is O(n^2) but best case is O(nlogn) in sorting.
 * This is mainly so I can obtain quick access to a tree. The programmer
 * needs to call getList to print a sorted list.
 * 
 * I may move this out later to deal with A*, SMA*, Alpha-Beta, BFS; DFS finding but HD
 * doesn't seem like the place for BFS and DFS path finding and SMA, alpha-beat and A * searches
 * would help in matching if fruitful.
 * 
 * Don't worry this class was done when running a ton of other programs to check their returns 
 * and after 5:30 pm.
 * 
 * @author aevens
 *
...
public class TreeSort<E>{
	...
	private void getInOrder(TreeNode root){
		...
		//visit the left until there are none, hit the node, get the greater node and repeat
		if(root != null){
	...
	private void getReversed(TreeNode root){
		...
		//get to greatest, then print greatest, then get next greatest by getting connected left value; etc.
		if(root != null){
	...
	/**
	 * Get In Postorder. This could be useful for
	 * testing.
	 * @return
	 */
	...
	/**
	 * Gets the BFS list. Iterate down to the level using the visited edges. Then back up be an increasing number of levels
	 * and then back down; etc. This is not dfs so the backtracking is based on a level of iteration where 2^(level) is 
	 * the number of nodes to potentially visit (checking at each level suffices). Each Previous level is checked for branches. 
	 * Termination is when no nodes are found for a certain traversal. I stored a parent but this can be done by iterating all the way down from the head (bleck!).
	 */
	private void itBreadthFirst(){
		...
		//current node will be null when both left and right have been visited and all are null
	...
	/**
	 * Gets a Breadth first Rep by storing level and iterating down to it 2^n nodes per level
	 * Speed was sacrificed for memory consumption incase the tree is huge | memory small
	 */
